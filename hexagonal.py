...
L_forward_x_s = L_forward_x
L_forward_y_s = L_forward_y
L_forward_z_s = L_forward_z
...
def FormObjectiveGradient(tao, x, G):
	...
	B = 2 * (h_v(rho) + h_s(rho))
	Ax = h_r(rho) * (lambda_r + 2 * mu_r) * tr(epsilon(ux))
	Ay = h_r(rho) * (lambda_r + 2 * mu_r) * tr(epsilon(uy))
	Az = h_r(rho) * (lambda_r + 2 * mu_r) * tr(epsilon(uz))
